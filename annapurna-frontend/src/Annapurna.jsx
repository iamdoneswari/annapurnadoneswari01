...
const Annapurna = () => {
    ...
    const renderDonorView = () => {
                        ...
                        <AnimatedSection className="space-y-6">
                            ...
                            {activeListings.map(listing => (
                                <div key={listing._id} className={`${cardClass} relative grid grid-cols-1 md:grid-cols-4 gap-4 items-start animate-fade-in`}>
                                    ...
                                    {/* --- START: Paste This New Status Tracker --- */}
                                    <div className="md:col-span-1 text-sm space-y-2">
                                        <p className="font-semibold mb-2">Status:</p>

                                        {/* --- New Static Stepper --- */}
                                        <div className="flex rounded-full overflow-hidden border border-gray-300 shadow-inner bg-gray-50">
                                            {/* Step 1: Available */}
                                            <div className={`flex-1 text-center py-1 px-2 text-xs font-bold transition-all duration-300
            ${listing.status === 'available'
                                                    ? 'bg-blue-600 text-white shadow'  // Active state
                                                    : 'bg-green-200 text-green-800'} // Completed state
        `}>
                                                Available
                                            </div>

                                            {/* Step 2: Claimed */}
                                            <div className={`flex-1 text-center py-1 px-2 text-xs font-bold transition-all duration-300 border-l border-r border-gray-300
            ${listing.status === 'claimed'
                                                    ? 'bg-blue-600 text-white shadow'  // Active state
                                                    : listing.status === 'picked_up'
                                                        ? 'bg-green-200 text-green-800' // Completed state
                                                        : 'bg-gray-200 text-gray-500'} // Pending state
        `}>
                                                Claimed
                                            </div>
        ...
        `}>
                                        ...
                                        {/* Rider Info (Keeps the same logic) */}
                                        {(listing.status === 'claimed' || listing.status === 'picked_up') && (() => {
                                            ...
                                            return order?.riderName
                                                ? (<div className="mt-3 text-xs"><p className="font-semibold">Rider Assigned:</p><p className="text-gray-700">{order.riderName}</p></div>)
                                                : <p className="mt-3 text-xs text-gray-500 animate-pulse">Awaiting Rider...</p>;
                                        })()}
                                    </div>
                                    {/* --- END: Paste This New Status Tracker --- */}
    ...
    // --- START: ENHANCED renderRiderView function ---
    // --- START: COMPLETE ENHANCED renderRiderView function ---
    const renderRiderView = () => {

        // --- Data Filtering & Calculation ---
        const myRiderOrders = dataForCurrentRole.myRiderOrders || []; // All orders assigned to this rider
        const availableOrdersForPickup = dataForCurrentRole.availableOrders || []; // Orders waiting for any rider

        // Separate MY orders based on status
        const myActiveDeliveries = myRiderOrders.filter(o => o.status === 'picked_up');
        const myCompletedDeliveries = myRiderOrders.filter(o => o.status === 'delivered');

        // Calculate earnings ONLY from COMPLETED deliveries
        const totalEarnings = myCompletedDeliveries.reduce((sum, order) => {
            return sum + (typeof order.riderPayout === 'number' && !isNaN(order.riderPayout) ? order.riderPayout : 0);
        }, 0);
        // --- END Data Filtering & Calculation ---


        // --- START: Enhanced OrderCard Component Definition ---
        const OrderCard = ({ order, isAvailable }) => {
            // Fee and Payout Calculation
            const totalFee = order.totalFee || 0;
            const riderPayout = order.riderPayout || 0;
            const annapurnaCommission = order.annapurnaCommission || 0;
            const listing = order.listingId; // Assumes listingId is populated

            // Basic check if listing data exists
            if (!listing || !listing.items || listing.items.length === 0) {
                console.warn("OrderCard missing valid listing data:", order._id);
                return (<div className="bg-red-50 p-3 rounded-lg text-red-700 text-sm border border-red-200 shadow-sm"> Error: Order data incomplete. </div>);
            }

            // Extract listing details
            const firstItemName = listing.items[0]?.itemName || 'Food Item';
            const dietaryIcon = listing.veg === 'veg' ? '🌱' : listing.veg === 'non-veg' ? '🍖' : '🍲';

            // --- Simulated Distance Calculation (Enhancement) ---
            let estimatedDistanceKm = null;
            if (isAvailable && receiverLocation.lat && receiverLocation.lng && listing.location?.coordinates) {
                // Calculate distance from receiver's set location to pickup
                ...
                // Also calculate distance from pickup to dropoff (simulated)
                // For a real app, you'd need receiver address coordinates stored in the order
                // Let's just add a small simulated extra distance for dropoff
                if (estimatedDistanceKm !== null) estimatedDistanceKm += Math.random() * 2 + 1; // Add 1-3km simulation
            }
            // --- End Simulated Distance ---


            // Function to generate map links
            const getMapLink = (address) => `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`;

            return (
                <div className={`bg-white rounded-xl shadow-lg border ${isAvailable ? 'border-gray-200 hover:border-blue-400 hover:shadow-md' : 'border-orange-200 shadow-orange-50'} transition duration-300 flex flex-col justify-between h-full overflow-hidden animate-fade-in`}>
                    {/* Card Header */}
                    <div className="p-4 border-b border-gray-100 bg-gray-50 rounded-t-xl">
                        <h4 className="font-semibold text-md sm:text-lg text-gray-800 truncate" title={firstItemName}>
                            {firstItemName} {listing.items.length > 1 ? <span className="text-xs font-normal text-gray-500">{`(+${listing.items.length - 1})`}</span> : ''} ({dietaryIcon})
                        </h4>
                    </div>

                    {/* Card Body */}
                    <div className="p-4 flex-grow">
                        {/* Addresses with Links */}
                        <div className="text-xs text-gray-600 space-y-2 mb-3">
                            <p className="flex items-start gap-1">
                                <span className="font-semibold text-gray-700 w-8 inline-block shrink-0">P/U:</span>
                                <a href={getMapLink(listing.address)} target="_blank" rel="noopener noreferrer" className="line-clamp-1 hover:text-blue-600 hover:underline" title={`Open map for: ${listing.address}`}>
                                    {listing.address || 'N/A'}
                                </a>
                            </p>
                            <p className="flex items-start gap-1">
                                <span className="font-semibold text-gray-700 w-8 inline-block shrink-0">D/O:</span>
                                <a href={getMapLink(order.receiverAddress)} target="_blank" rel="noopener noreferrer" className="line-clamp-1 hover:text-blue-600 hover:underline" title={`Open map for: ${order.receiverAddress}`}>
                                    {order.receiverAddress || 'N/A'}
                                </a>
                            </p>
                            {/* Estimated Distance */}
                            {isAvailable && estimatedDistanceKm !== null && (
                                <p className="font-semibold text-indigo-600 text-xs mt-1">~ {estimatedDistanceKm.toFixed(1)} km est. total distance</p>
                            )}
                            {isAvailable && !estimatedDistanceKm && !receiverLocation.lat && (
                                <p className="text-xs text-gray-400 mt-1">Set your location to see distance</p>
                            )}
                        </div>

                        {/* Payout/Fee Info */}
                        <div className="text-xs bg-gradient-to-r from-gray-50 to-gray-100 p-3 rounded-lg border border-gray-200 mt-3 shadow-inner">
                            <p className="font-bold text-green-600 text-lg mb-1 text-center">Your Payout: ₹{riderPayout.toFixed(2)}</p>
                            <div className="flex justify-between text-gray-500 border-t pt-1 mt-1">
                                <span>Total Fee: ₹{totalFee.toFixed(2)}</span>
                                <span>Commission: ₹{annapurnaCommission.toFixed(2)}</span>
                            </div>
                        </div>
                    </div>

                    {/* Action Button Footer */}
                    <div className="p-4 bg-gray-100 border-t border-gray-200 rounded-b-xl"> {/* Footer styling */}
                        {isAvailable ? (
                            <button
                                onClick={() => handleRiderAccept(order._id)}
                                disabled={loading || dataLoading}
                                className="w-full bg-blue-600 hover:bg-blue-700 text-white py-2.5 rounded-lg font-bold text-sm disabled:opacity-50 transition duration-200 shadow hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75"
                            >
                            ...
                            <button
                                ...
                                disabled={loading || dataLoading}
                                className="w-full bg-green-600 hover:bg-green-700 text-white py-2.5 rounded-lg font-bold text-sm disabled:opacity-50 transition duration-200 shadow hover:shadow-md focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-opacity-75"
                            >
                                {loading ? 'Completing...' : 'MARK DELIVERED'}
                            </button>
                        )}
                        {!isAvailable && <p className="text-xs text-center font-semibold text-orange-600 mt-2">Status: {order.status.replace('_', ' ').toUpperCase()}</p>}
                    </div>
                </div>
            );
        };
        // --- END: Enhanced OrderCard Component Definition ---


        // --- START: JSX Return for renderRiderView ---
        return (
            // Main container with professional background gradient
            <div className="container mx-auto px-4 py-8 pt-24 min-h-screen bg-gradient-to-b from-gray-50 to-indigo-100">

                {/* Dashboard Header */}
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 px-2">
                    <div>
                        <h2 className="text-3xl sm:text-4xl font-extrabold text-gray-900">Rider Hub</h2>
                        <p className="text-md text-gray-600 mt-1">Manage deliveries, track your impact and earnings.</p>
                    </div>
                    <div className="mt-4 sm:mt-0 relative inline-block px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-700 text-white rounded-lg shadow-xl transform hover:scale-105 transition-transform duration-200">
                        <span className="font-semibold text-sm tracking-wide">🚀 Annapurna Delivery Partner</span>
                    </div>
                </div>

                {/* Error Display */}
                {error && <p className="bg-red-100 border-l-4 border-red-500 text-red-700 px-4 py-3 rounded-md mb-6 text-sm shadow-md">{error}</p>}

                {/* Main Content Area - Grid Layout */}
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">

                    {/* --- Left Column (Span 4/12) --- */}
                    <div className="lg:col-span-4 space-y-6">
                        {/* Earnings Summary Card */}
                        <section className="bg-white p-6 rounded-xl shadow-xl border border-gray-100 sticky top-24 transform transition-transform duration-300 hover:-translate-y-1">
                            ...
                            <div className="text-center bg-gradient-to-br from-green-50 to-emerald-100 p-5 rounded-lg border border-green-200 shadow-inner">
                                <p className="text-sm text-emerald-800 font-semibold uppercase tracking-wider">Completed Earnings</p>
                                <p className="text-5xl font-extrabold text-emerald-700 mt-2 mb-1">₹{totalEarnings.toFixed(2)}</p>
                                <p className="text-xs text-gray-500">From {myCompletedDeliveries.length} delivered order{myCompletedDeliveries.length !== 1 ? 's' : ''}</p>
                            </div>
                            {/* Optional: Add link to detailed earnings report */}
                            {/* <a href="#" className="text-xs text-blue-600 hover:underline mt-4 block text-center">View Details</a> */}
                        </section>

                        {/* Active Delivery Section */}
                        <section className="bg-white p-4 sm:p-6 rounded-xl shadow-xl border border-gray-100">
                            <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center border-b pb-3">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mr-2 text-orange-500 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}> <path strokeLinecap="round" strokeLinejoin="round" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                                Active Delivery ({myActiveDeliveries.length})
                            </h3>
                            <div className="space-y-4">
                                {dataLoading && myActiveDeliveries.length === 0 && myRiderOrders.length === 0 && <p className="text-gray-500 animate-pulse text-sm py-4 text-center">Loading...</p>}
                                {!dataLoading && myActiveDeliveries.length === 0 && <p className="text-gray-500 text-sm py-4 text-center">No active deliveries.</p>}
                                {myActiveDeliveries.map(order => <OrderCard key={order._id} order={order} isAvailable={false} />)}
                            </div>
                        </section>
                    </div> {/* End Left Column */}

                    {/* --- Right Column (Span 8/12) --- */}
                    <div className="lg:col-span-8 space-y-6">
                        {/* Available Orders Section */}
                        <section className="bg-white p-4 sm:p-6 rounded-xl shadow-xl border border-gray-100">
                            <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center border-b pb-3">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mr-2 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h7" /></svg>
                                Available Orders ({availableOrdersForPickup.length})
                            </h3>
                            {dataLoading && availableOrdersForPickup.length === 0 && <p className="text-gray-500 animate-pulse text-sm py-4 text-center">Checking for orders...</p>}
                            {!dataLoading && availableOrdersForPickup.length === 0 && <p className="text-gray-500 text-sm py-4 text-center">No orders currently available for pickup.</p>}
                            {/* Grid layout for available orders */}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
                        ...
                        <section className="bg-white p-4 sm:p-6 rounded-xl shadow-xl border border-gray-100">
                            ...
                            {dataLoading && myCompletedDeliveries.length === 0 && myRiderOrders.length === 0 && <p className="text-gray-500 animate-pulse text-sm py-4 text-center">Loading history...</p>}
                            {!dataLoading && myCompletedDeliveries.length === 0 && <p className="text-gray-500 text-sm py-4 text-center">No completed deliveries yet.</p>}
                            {/* History List */}
                            <div className="space-y-3 max-h-96 overflow-y-auto pr-2 custom-scrollbar">
                                {myCompletedDeliveries.map(order => {
                                    const listingDetails = order.listingId || { items: [{ itemName: 'Unknown' }], address: 'N/A' };
                                    const firstItemName = listingDetails.items?.[0]?.itemName || 'Food';
                                    return (
                                        <div key={order._id} className="bg-gray-50 p-3 rounded-lg border border-gray-200 flex flex-col sm:flex-row justify-between sm:items-center gap-3 transition duration-200 hover:bg-gray-100 shadow-sm">
                                            <div className="flex-grow">
                                                <h4 className="font-semibold text-sm text-gray-800">{firstItemName} {listingDetails.items?.length > 1 ? `(+${listingDetails.items.length - 1})` : ''}</h4>
                                                <p className="text-gray-500 text-xs mt-1 line-clamp-1" title={`To: ${order.receiverName || 'N/A'} | P/U: ${listingDetails.address}`}>To: {order.receiverName || 'N/A'} | P/U: {listingDetails.address}</p>
                                                <p className="text-xs text-gray-400">Delivered: {order.deliveredAt ? new Date(order.deliveredAt).toLocaleString() : 'N/A'}</p>
                                            </div>
                                            <div className="text-left sm:text-right mt-2 sm:mt-0 flex-shrink-0">
                                                <p className="text-lg font-bold text-green-600">₹{order.riderPayout?.toFixed(2) || '0.00'}</p>
        ...
        // --- END: JSX Return for renderRiderView ---
    };
    // --- END: COMPLETE ENHANCED renderRiderView function ---END: COMPLETE REPLACEMENT for renderRiderView function ---
